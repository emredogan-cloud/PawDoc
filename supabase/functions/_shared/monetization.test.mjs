...
// --- One plan: any active entitlement maps to premium -----------------------
test("any entitlement id maps to premium (one plan)", () => {
  assert.equal(
    entitlementStatusFromEvent({ type: "INITIAL_PURCHASE", entitlement_ids: ["premium"] }),
    "premium",
  );
  assert.equal(
    entitlementStatusFromEvent({ type: "RENEWAL", entitlement_ids: ["legacy_pro"] }),
    "premium",
  );
});

test("trial period always maps to trial regardless of entitlement id", () => {
  assert.equal(
