...
|---|---|---|---|
| `AI_SERVICE_TOKEN` 🔒 ⏳ | **Phase A trust boundary.** Shared bearer secret: the AI service (`pawdoc-ai`) requires `Authorization: Bearer <token>` on `/analyze`, `/embed`, `/generate_journal` (constant-time compare; `/health` stays open). The Edge Functions present it. **Same value on BOTH sides.** Prod fails CLOSED (503) if unset. Mint a high-entropy value (`openssl rand -hex 32`), store in Doppler `prd`, then sync to **Fly env** (AI service) **and** `supabase secrets set` (Edge Functions). | Yes (prod) | Fly env on `pawdoc-ai` **and** `supabase secrets set` (analyze / analyze-anonymous / generate-journals) |
...
| `PET_INSURANCE_AFFILIATE_URL` | Pet-insurance affiliate URL (Trupanion / Healthy Paws / ...). CTA SELF-HIDES if empty — Phase 6.3 | Optional (revenue share) | `--dart-define=PET_INSURANCE_AFFILIATE_URL=…` on `flutter build` |
| `RESEND_API_KEY` 🔒 | Resend transactional-email key for the Family Sharing invite emails. Empty = the Edge logs the magic link and returns it in the response body (founder-friendly fallback) — Phase 6.3.1 | Optional (email delivery) | `supabase secrets set` on `invite-family-member` |
| `RESEND_FROM` | Sender identity for the invite email (e.g. `PawDoc <noreply@pawdoc.app>`) — Phase 6.3.1 | Optional | `supabase secrets set` on `invite-family-member` |
| `INVITE_LINK_BASE_URL` | Base URL for the family invite magic link (default `https://pawdoc.app/invite`). Must match the Universal-Link / App-Link target — Phase 6.3.1 | Optional | `supabase secrets set` on `invite-family-member` |
...
>
> **Phase 6.1 (Personalization Engine):** no new env vars or secrets. The Edge `/analyze` Function fetches the pet's last 30 days of `analyses` + `health_events` under the user's JWT (RLS-scoped — no service-role widening); compact summaries are forwarded to the AI service in `recent_analyses` / `recent_events`. The Claude prompt now carries **two** ephemeral `cache_control` breakpoints (static safety contract + per-pet personalization block) so a 2nd check on the same pet within 5 minutes pays only 25% of input cost on the cached prefix (Anthropic prompt caching). The golden-set eval (`scripts/run-eval.py`, `ai-service/tests/golden_set.json`) is the **CR #2-eval safety gate** — `pytest` and `verify-phase-6.1.sh` both fail the build if any EMERGENCY case regresses.
>
> **Phase 6.2 (Outcome Feedback Loop & Data Foundation):** no new server secrets. The DB now CHECKs `analysis_feedback.outcome` to the 5 canonical values; the admin-only views `view_accuracy_signals` + `view_accuracy_summary` join analyses↔feedback and flag FP / FN / TP / TN — REVOKE from anon/authenticated, GRANT only to service_role. The dataset export CLI (`scripts/export-training-dataset.py`) uses the existing `SUPABASE_SERVICE_ROLE_KEY` to extract joined rows over PostgREST and writes a fine-tune-ready JSONL with a strict PII allowlist + `assert_no_pii` guard; `*.jsonl` is `.gitignore`d as belt-and-braces. Founder should run the export weekly and feed the false-negative-proxy rows into `ai-service/tests/golden_set.json` so the safety eval grows from real incidents.
>
> **Phase 6.3 (Family Sharing + PDF Health Report + Insurance affiliate):** no new server secrets. The Family Sharing migration redesigns RLS to be *per-family-group* via two `SECURITY DEFINER` helpers (`is_family_member`, `is_family_pet`) that bypass RLS on `family_members` to avoid recursion; every existing + new user gets a default solo group via backfill + an after-insert trigger, so standalone users keep the same RLS behavior they had pre-migration. The $4.99 PDF Health Report add-on is enforced server-side by the new `users.pdf_reports_remaining` column and the `/generate-pdf-report` Edge Function; RevenueCat consumable product `pdf_report_addon` increments the counter via the webhook. The PDF is **never persisted** — generated inline with `pdf-lib` and streamed to the client (`Cache-Control: no-store`, `Content-Disposition: attachment`). `PET_INSURANCE_AFFILIATE_URL` is a new optional build-define; the CTA self-hides when empty.
>
> **Phase 6.3.1 (Family Sharing UI & Invites):** adds the `family_invites` table (48-hour token, status enum, inviter-only SELECT, service-role-only writes) and two Edge Functions. `/invite-family-member` (verify_jwt=true) checks the inviter is on `family` or `b2b_lite` (server-enforced, not just client), generates a 32-byte URL-safe token, inserts a pending row, optionally sends the magic link via Resend (degrades gracefully when `RESEND_API_KEY` is empty — logs the link + returns it). `/accept-family-invite` (verify_jwt=true) validates the token, blocks invitees already in a shared family via the new `count_shared_group_memberships()` `SECURITY DEFINER` helper, and is idempotent on retries. Client: `FamilySettingsScreen` + `InviteFamilyMemberScreen` + `AcceptFamilyInviteScreen` + go_router `/invite/:token` route + `PendingInvitePrefs` so the token survives the sign-in detour.
>
